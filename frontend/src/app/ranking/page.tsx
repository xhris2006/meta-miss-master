...
export default function RankingPage() {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: "Reines du Meta — Classement", url });
      } catch {}
    } else {
  ...
  const handleWhatsApp = () => {
    window.open(
      `https://wa.me/?text=${encodeURIComponent(
        "Classement Reines du Meta 2026 : " + window.location.href
      )}`,
      "_blank",
      "noopener,noreferrer"
    );
  ...
  const handleTwitter = () => {
    window.open(
      `https://twitter.com/intent/tweet?text=${encodeURIComponent(
        "Classement Reines du Meta 2026"
              ...
              <div key={c.id} className={`rk-ccard fade-up${i === 0 ? " top" : ""}`} style={{ animationDelay: `${Math.min(i, 8) * 0.05}s` }}>
                ...
                <div className="rk-cactions">
                  ...
                  <button className="rk-iconbtn rk-wa" aria-label="WhatsApp" onClick={() => {
                    const url = `${window.location.origin}/candidates/${c.id}`;
                    window.open(`https://wa.me/?text=${encodeURIComponent(`Votez pour ${c.name} sur Reines du Meta ! ${url}`)}`, "_blank", "noopener,noreferrer");
                  }}>
